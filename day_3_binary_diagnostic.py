...
def day_3_task_1():
	ones = None
	n, nb = 0, 0
	with open('day_3_binary_diagnostic.in') as f:
	...
	gama = ''.join([str([0, 1][b > n//2]) for b in ones])
	power = ''.join([str([1, 0][b > n//2]) for b in ones])
	print(int(gama, 2)*int(power, 2))
...
def day_3_task_2():
	...
	ones = ones >= ceil(len(nums)/2)
	for nn in nums:
		if (ones and nn[0] == '1') or (not ones and nn[0] == '0'):
			ogr += [nn]
		else:
			co2 += [nn]
	...
	while len(ogr) > 1:
		...
		for b in ogr:
			ones += b[i] == '1'
		ones = ones >= ceil(len(ogr)/2)

		tmp = []
		for b in ogr:
			if (ones and b[i] == '1') or (not ones and b[i] == '0'):
				tmp += [b]
		ogr = tmp
		i += 1
	...
	while len(co2) > 1:
		...
		for b in co2:
			ones += b[i] == '1'
		ones = ones >= ceil(len(co2)/2)

		tmp = []
		for b in co2:
			if (ones and b[i] == '0') or (not ones and b[i] == '1'):
				tmp += [b]
		co2 = tmp
		i += 1
