...
def day_20_task_1():
	...
	image = []
	with open('day_20_trench_map.in') as f:
		enhancement_algorithm = f.readline().replace('\n', '')
		f.readline()
			...
			for j in range(-1, len(image[0]) + 1):
				...
				for k in range(9):
					...
					if x < 0 or x >= len(image) or y < 0 or y >= len(image[x]):
						...
						n = n*2 + 1
				new_line += enhancement_algorithm[n]
			new_image += [new_line]
		image = new_image
		if enhancement_algorithm[0] == '#':
			outside = (outside + 1)%2

	light = sum([sum([0, 1][x == '#'] for x in l) for l in image])
	print(light)
