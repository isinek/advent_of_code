...
'''
def day_13_task_1():
	with open('day_13_shuttle_search.in') as f:
		earliest_depart = int(f.readline())
		buses = [int(n) for n in f.readline().split(',') if n != 'x']

	earliest_bus = (earliest_depart*20, 99999)
	for b in buses:
		tmp_earliest = (earliest_depart//b + 1)*b
		if tmp_earliest < earliest_bus[0]:
			earliest_bus = (tmp_earliest, b)

	print((earliest_bus[0] - earliest_depart)*earliest_bus[1])
...
def day_13_task_2():
	...
	with open('day_13_shuttle_search.in') as f:
		...
		for i, n in enumerate(f.readline().split(',')):
			if n != 'x':
				buses += [(int(n), i)]
				if slowest_bus[0] < int(n):
					slowest_bus = buses[-1]

	cycle = [buses[0][0], buses[0][1]]
	curr_timestamp = cycle[0] - cycle[1]
	for b, j in buses[1:]:
		new_cycle = [0, 0]
		while True:
			if not (curr_timestamp + j)%b:
				if not new_cycle[1]:
					new_cycle[1] = curr_timestamp
				else:
					new_cycle[0] = curr_timestamp - new_cycle[1]
					cycle = new_cycle
					break
			curr_timestamp += cycle[0]
