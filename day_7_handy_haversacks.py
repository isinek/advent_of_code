...
'''
def look_into_bag(bags, my_bag, bag):
	bags[bag]['contains_my_bag'] = False
	for curr_bag in bags[bag]['bags']:
		if curr_bag['bag'] == my_bag:
			bags[bag]['contains_my_bag'] = True
			break
		elif not bags[curr_bag['bag']]['contains_my_bag'] is None and bags[curr_bag['bag']]['contains_my_bag']:
			bags[bag]['contains_my_bag'] = True
			break
		elif bags[curr_bag['bag']]['contains_my_bag'] is None:
			bags, contains = look_into_bag(bags, my_bag, curr_bag['bag'])
			bags[bag]['contains_my_bag'] |= contains
	return bags, bags[bag]['contains_my_bag']

def day_7_task_1():
	bags = {}
	my_bag = 'shiny gold'
	bag_queue = []
	with open('day_7_handy_haversacks.in') as f:
		for line in f.readlines():
			words = line.strip().split()
			curr_bag = words[0] + ' ' + words[1]
			bags[curr_bag] = {'bags': [], 'contains_my_bag': None}
			i = 1
			while i*4 < len(words):
				if words[i*4] != 'no':
					bags[curr_bag]['bags'] += [{'n': int(words[i*4]), 'bag': words[i*4 + 1] + ' ' + words[i*4 + 2]}]
				i += 1

			bag_queue += [curr_bag]
	for curr_bag in bag_queue:
		if curr_bag == my_bag or bags[curr_bag]['contains_my_bag']:
			continue
		bags, contains = look_into_bag(bags, my_bag, curr_bag)

	contain_my_bag = sum([1 for x in bags if bags[x]['contains_my_bag']])
	print(contain_my_bag)
...
'''
def bags_inside(bags, curr_bag):
	res = 1
	for x in bags[curr_bag]['bags']:
		res += x['n']*bags_inside(bags, x['bag'])
	return res
...
def day_7_task_2():
	...
	print(bags_inside(bags, my_bag) - 1)
