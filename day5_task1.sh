...
function is_in_right_order() {
  ...
  for i in $( seq 0 $(( ${#pages[@]} - 2 )) ); do
    for j in $( seq $(( i + 1 )) $(( ${#pages[@]} - 1 )) ); do
      if echo "${rules[*]}" | grep -Poq "(${pages[${j}]}\|${pages[${i}]})"; then
        return 1
      fi
    done
  done
...
function main() {
  ...
  while read -r line; do
    ...
    if is_in_right_order "${input_file}" "${pages[*]}"; then
      sum=$(( sum + ${pages[$(( ${#pages[@]}/2 ))]} ))
    fi
