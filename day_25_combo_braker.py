...
'''
def day_25_task_1():
	with open('day_25_combo_braker.in') as f:
	...
	for i in range(1, 1000000000):
		tmp = (tmp*7)%20201227
		if loop_sizes[0] == 0 and public_keys[0] == tmp:
			loop_sizes[0] = i
		if loop_sizes[1] == 0 and public_keys[1] == tmp:
			loop_sizes[1] = i
		if not 0 in loop_sizes:
			break

	tmp = 1
	for i in range(loop_sizes[1]):
		tmp = (tmp*public_keys[0])%20201227
