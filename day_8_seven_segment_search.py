...
def day_8_task_1():
	res = 0
	with open('day_8_seven_segment_search.in') as f:
		for line in f.readlines():
			out = line.split(' | ')[1].split()
			res += sum([1 for x in out if len(x) in [2, 3, 4, 7]])
	print(res)
...
four-digit output values. What do you get if you add up all of the
output values?
'''
def day_8_task_2():
	nums = []
		...
		for line in f.readlines():
			...
			while len(inp):
				...
				if len(x) == 2:
					...
					patts[8] = x
				elif not patts[7] is None and len(x) == 5 and all([[0, 1][s in x] for s in patts[7]]):
					patts[3] = x
				elif not patts[9] is None and len(x) == 5 and all([[0, 1][s in patts[9]] for s in x]):
					patts[5] = x
				elif not patts[3] is None and not patts[5] is None and len(x) == 5:
					patts[2] = x
				elif not patts[3] is None and len(x) == 6 and all([[0, 1][s in x] for s in patts[3]]):
					patts[9] = x
				elif not patts[5] is None and len(x) == 6 and all([[0, 1][s in x] for s in patts[5]]):
					patts[6] = x
				elif not patts[9] is None and not patts[6] is None and len(x) == 6:
					patts[0] = x
				else:
					inp += [x]
			...
			nums += [0]
			for x in out:
				for n, p in enumerate(patts):
					if len(x) == len(p) and all([[0, 1][s in p] for s in x]):
						nums[-1] = nums[-1]*10 + n
