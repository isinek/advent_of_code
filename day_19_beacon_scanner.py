...
def day_19_task_1():
	scanners = []
	beacons = []
	with open('day_19_beacon_scanner.in') as f:
		for line in f.readlines():
			...
			beacons[-1] += [{int(x) for x in line.split(',')}]

	overlapping_beacons = [[[0, len(beacons[i])][i == j] for j in range(len(scanners))] for i in range(len(scanners))]
