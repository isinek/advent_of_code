...
func DailRotation(position int, left bool, steps int) (int, uint64) {
	zeros := uint64(steps / 100)
	steps %= 100
	if steps == 0 {
...
func SolutionDay1Task1(inputFile string) (uint64, error) {
	...
	for scanner.Scan() {
		...
		dail, _ = DailRotation(dail, left, steps)
		if dail == 0 {
			result += 1
		}
	}
...
func task1() {
	result, err := SolutionDay1Task1("days/day01.in")
	if err != nil {
		fmt.Printf("error: %v\n", err)
		return
	}
...
func SolutionDay1Task2(inputFile string) (uint64, error) {
	file, err := os.Open(inputFile)
	if err != nil {
		return 0, err
	}
	...
	for scanner.Scan() {
		...
		dail, zeros = DailRotation(dail, left, steps)
		result += zeros
	}
	if err := scanner.Err(); err != nil {
		return 0, err
...
func task2() {
	result, err := SolutionDay1Task2("days/day01.in")
	if err != nil {
		fmt.Printf("error: %v\n", err)
		return
	}
