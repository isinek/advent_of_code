...
def day_17_task_1():
	x, y = [], []
	with open('day_17_trick_shot.in') as f:
		line = f.readline().replace('target area: ', '').replace('x=', '').replace('y=', '')
		x, y = line.split(', ')
	...
	for cy in yy:
		for cx in xx:
			if cy[1] == cx[1] or (cy[1] > cx[1] and cx[2] == 0):
				sol += [(cx[0], cy[0], cy[3])]
				break
	print(sol[0][2])
...
def day_17_task_2():
	...
	for cy in yy:
		for cx in xx:
			if (cy[1] == cx[1] or (cy[1] > cx[1] and cx[2] == 0)) and not (cx[0], cy[0]) in sol:
				sol += [(cx[0], cy[0])]
	print(len(sol))
