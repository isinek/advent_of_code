...
'''
def execute_boot(boot_code):
	acc = 0
	inst_queue = [0]
	while inst_queue[-1] < len(boot_code):
		i = inst_queue[-1]
		if boot_code[i]['executed']:
			break

		boot_code[i]['executed'] = True
		if boot_code[i]['instruction'] == 'acc':
			acc += boot_code[i]['value']
		elif boot_code[i]['instruction'] == 'jmp':
			inst_queue += [i + boot_code[i]['value']]
			continue
		inst_queue += [i + 1]

	return acc, boot_code, inst_queue


def day_8_task_1():
	boot_code = []
	with open('day_8_handheld_halting.in') as f:
		for line in f.readlines():
			ins, val = line.strip().split()
			boot_code += [{'instruction': ins, 'value': int(val), 'executed': False}]

	res = execute_boot(boot_code)
	print(res[0])
...
def day_8_task_2():
	...
	acc, boot_code, inst_queue = execute_boot(boot_code)

	for i in inst_queue:
		boot_code[i]['executed'] = False

	for i in inst_queue[::-1]:
		run = boot_code[i]['instruction'] == 'nop' or boot_code[i]['instruction'] == 'jmp'
		if run:
			tmp_boot_code = [deepcopy(x) for x in boot_code]
			tmp_boot_code[i]['instruction'] = ['jmp', 'nop']['jmp' == tmp_boot_code[i]['instruction']]
			acc, tmp_boot_code, tmp = execute_boot(tmp_boot_code)
			if tmp_boot_code[-1]['executed']:
				break
