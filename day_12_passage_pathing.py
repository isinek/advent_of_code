...
def day_12_task_1():
	caves = {}
	with open('day_12_passage_pathing.in') as f:
		for line in f.readlines():
			line = line.replace('\n', '')
			a, b = line.split('-')
			if not a in caves:
				caves[a] = { 'small': a == a.lower(), 'paths': [b] }
			else:
				caves[a]['paths'] += [b]

			if not b in caves:
				caves[b] = { 'small': b == b.lower(), 'paths': [a] }
			else:
				caves[b]['paths'] += [a]
	...
	while len(q):
		path = q.pop(0)
		current_cave = caves[path[-1]]
		for p in current_cave['paths']:
			if p == 'end':
				paths += [path + [p]]
			elif not caves[p]['small'] or not p in path:
				q += [path + [p]]
...
def day_12_task_2():
	...
	with open('day_12_passage_pathing.in') as f:
		for line in f.readlines():
			line = line.replace('\n', '')
			a, b = line.split('-')
			if not a in caves and b != 'start':
				caves[a] = { 'small': a == a.lower(), 'paths': [b] }
			elif b != 'start':
				caves[a]['paths'] += [b]

			if not b in caves and a != 'start':
				caves[b] = { 'small': b == b.lower(), 'paths': [a] }
			elif a != 'start':
				caves[b]['paths'] += [a]
	...
	while len(q):
		...
		for p in current_cave['paths']:
			...
			elif not caves[p]['small'] or not has_double or not exists:
				new_double = (caves[p]['small'] and exists) or has_double
				q += [new_path]
				d += [new_double]
