...
def day_21_task_1():
	allergens = {}
	foods = {}
	with open('day_21_allergen_assessment.in') as f:
		for line in f.readlines():
			fs, alls = [[y.strip(',').strip(')') for y in x.split()] for x in line.strip('\n').split(' (contains ')]
			for a in alls:
				if not a in allergens:
					allergens[a] = {}
				for f in fs:
					if not f in allergens[a]:
						allergens[a][f] = 1
					else:
						allergens[a][f] += 1
			for f in fs:
		...
		allrgns_queue = list(allergens.keys())
		while len(allrgns_queue):
			a = allrgns_queue.pop(0)
			multiple_max = False
			max_val = (0, 0)
			for f in allergens[a]:
				if not f in foods:
					continue
				if max_val[1] < allergens[a][f]:
					max_val = (f, allergens[a][f])
					multiple_max = False
				elif max_val[1] == allergens[a][f]:
					multiple_max = True
			if not multiple_max:
				foods.pop(max_val[0])
			else:
		...
		print(sum([foods[f] for f in foods]))
...
def day_21_task_2():
	...
	with open('day_21_allergen_assessment.in') as f:
		...
		while len(allrgns_queue):
			...
			if not multiple_max:
				foods.pop(max_val[0])
				for f in list(allergens[a].keys()):
					if f != max_val[0]:
						allergens[a].pop(f)
			else:
				allrgns_queue += [a]

		allrgns = sorted(list(allergens.keys()))
		print(','.join([list(allergens[a].keys())[0] for a in allrgns]))
