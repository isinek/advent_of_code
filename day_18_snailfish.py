...
'''
def reducing(snailfish):
	new_changes = True
	while new_changes:
		highest = len(sorted(snailfish, key=lambda x: -len(x[1]))[0][1])

		i = 0
		new_changes = False
		while i < len(snailfish):
			if highest > 4 and len(snailfish[i][1]) == highest and snailfish[i][1][-1] == 'L':
				if i - 1 >= 0:
					snailfish[i - 1][0] += snailfish[i][0]
				snailfish[i][0] = 0
				snailfish[i][1] = snailfish[i][1][:-1]
			elif highest > 4 and len(snailfish[i][1]) == highest and snailfish[i][1][-1] == 'R':
				if i + 1 < len(snailfish):
					snailfish[i + 1][0] += snailfish[i][0]
				del snailfish[i]
				new_changes = True
				break
			elif highest < 5 and snailfish[i][0] >= 10:
				new_l = [snailfish[i][0]//2, snailfish[i][1] + 'L']
				new_r = [snailfish[i][0] - new_l[0], snailfish[i][1] + 'R']
				snailfish = snailfish[:i] + [new_l, new_r] + snailfish[i + 1:]
				new_changes = True
				break
			i += 1
...
def magnitude(snailfish):
	...
	while len(snailfish) > 1:
		if snailfish[i][1][-1] == 'R' and snailfish[i - 1][1] == snailfish[i][1][:-1] + 'L':
			snailfish[i - 1][1] = snailfish[i][1][:-1]
			snailfish[i - 1][0] = 3*snailfish[i - 1][0] + 2*snailfish[i][0]
			del snailfish[i]
			i -= 1
		i = (i + 1)%len(snailfish)
...
def day_18_task_1():
	snailfish = []
	with open('day_18_snailfish.in') as f:
		for line in f.readlines():
			line = line.replace('\n', '')

			for i in range(len(snailfish)):
				snailfish[i][1] = 'L' + snailfish[i][1]
...
def day_18_task_2():
	...
	max_m = 0
	for i, sl in enumerate(snailfishes):
		for j, sr in enumerate(snailfishes):
			if i == j:
				continue

			s = reducing([[x[0], 'L' + x[1]] for x in sl] + [[x[0], 'R' + x[1]] for x in sr])
			m = magnitude(s)
			if m > max_m:
				max_m = m
