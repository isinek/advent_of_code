...
'''
def day_22_task_1():
	cubes = [[[0 for z in range(-50, 51)] for y in range(-50, 51)] for x in range(-50, 51)]
	with open('day_22_reactor_reboot.in') as f:
		for line in f.readlines():
			on_off, coords = line.replace('\n', '').split()
			coords = [[int(n) for n in x.split('..')] for x in coords.replace('x=', '').replace('y=', '').replace('z=', '').split(',')]
			if any([(x[0] < -50 and x[1] < -50) or (x[0] > 50 and x[1] > 50) for x in coords]):
				continue
			if on_off == 'on':
				for x in range(max([-50, coords[0][0]]), min([51, coords[0][1] + 1])):
					for y in range(max([-50, coords[1][0]]), min([51, coords[1][1] + 1])):
						for z in range(max([-50, coords[2][0]]), min([51, coords[2][1] + 1])):
							cubes[x][y][z] = 1
			else:
				for x in range(max([-50, coords[0][0]]), min([51, coords[0][1] + 1])):
					for y in range(max([-50, coords[1][0]]), min([51, coords[1][1] + 1])):
				...
				for x in range(coords[0][0], coords[0][1] + 1):
					...
					for y in range(coords[1][0], coords[1][1] + 1):
						if not y in cubes[x]:
							continue
						for z in range(coords[2][0], coords[2][1] + 1):
							if not z in cubes[x][y]:
								continue
							del cubes[x][y][z]

	print(sum([sum([sum(cubes[x][y]) for y in cubes[x]]) for x in cubes]))
