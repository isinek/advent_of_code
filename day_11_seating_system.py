...
ccupied, the seat becomes empty.
...
'''
def day_11_task_1():
	with open('day_11_seating_system.in') as f:
	...
	while change_occured:
		...
		new_layout = [[x for x in line] for line in layout]
		for i in range(len(layout)):
			for j in range(len(layout[i])):
				...
				tmp_count = 0
				for x in range(9):
					if not (0 <= i + x//3 - 1 < len(layout) and 0 <= j + x%3 - 1 < len(layout[i])):
						continue
					if layout[i + x//3 - 1][j + x%3 - 1] == '#':
						tmp_count += 1
				if layout[i][j] == 'L' and tmp_count == 0:
					new_layout[i][j] = '#'
					occupied_seats += 1
					change_occured = True
				elif layout[i][j] == '#' and tmp_count >= 5:
					new_layout[i][j] = 'L'
					occupied_seats -= 1
					change_occured = True
		layout = new_layout
		...
		for j in range(len(layout[i])):
			...
			for x in range(1, len(layout)):
				if i - x < 0:
					...
					visible_seats[-1] += [(i - x, j)]
					break

			for x in range(1, len(layout)):
				if i - x < 0 or j + x >= len(layout[i]):
					break
				elif layout[i - x][j + x] != '.':
					visible_seats[-1] += [(i - x, j + x)]
					break
			...
			for x in range(1, len(layout)):
				if i + x >= len(layout) or j + x >= len(layout[i]):
					break
				elif layout[i + x][j + x] != '.':
					visible_seats[-1] += [(i + x, j + x)]
					break
		...
		for i in range(len(layout)):
			for j in range(len(layout[i])):
				if layout[i][j] == '.':
					continue
				tmp_count = 0
				for x in visible_seats[i*len(layout[i]) + j]:
					if layout[x[0]][x[1]] == '#':
						tmp_count += 1
