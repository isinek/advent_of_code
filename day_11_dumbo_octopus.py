...
def day_11_task_1():
    nums = []
    flashes = 0
    with open('day_11_dumbo_octopus.in') as f:
            ...
            if nums[x][y] > 9:
                ...
                for i in range(-1, 2):
                    for j in range(-1, 2):
                        ...
                        xx = x + i
                        yy = y + j
                        if xx >= 0 and xx < len(nums) and yy >= 0 and yy < len(nums[xx]):
                            nums[xx][yy] += 1
                            if nums[xx][yy] > 9 and not (xx, yy) in fs and not (xx, yy) in q:
                                q += [(xx, yy)]
        ...
        for i in range(len(nums)):
            for j in range(len(nums[i])):
                if nums[i][j] > 9:
                    flashes += 1
                    nums[i][j] = 0
        if flashes == len(nums)*len(nums[0]):
            print(n)
            return
