...
def day_18_task_1():
	with open('day_18_operation_order.in') as f:
		res = 0
		for line in f.readlines():
			line = format_line(line)
			polish = reverse_polish_notation(line)
			res += calculation(polish)
...
'''
def reverse_polish_notation_v2(arr):
	polish = [[]]
	operation = [[]]
	...
	for x in arr:
		if x == '(':
			...
			while len(operation[-1]) and operation[-1][-1] == '+':
				polish[-1] += operation[-1].pop()
			operation[-1] += [x]
		else:
			polish[-1] += [int(x)]
...
def day_18_task_2():
	with open('day_18_operation_order.in') as f:
		...
		for line in f.readlines():
			line = format_line(line)
			polish = reverse_polish_notation_v2(line)
			res += calculation(polish)
