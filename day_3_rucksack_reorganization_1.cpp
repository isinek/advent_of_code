...
int main()
{
	string rucksack, comp1, comp2;
	unsigned long prio_sum = 0;

	for (short i = 0; i < N_INPUTS; ++i) {
		cin >> rucksack;

		comp1 = rucksack.substr(0, rucksack.length()/2);
		comp2 = rucksack.substr(rucksack.length()/2);

		for (char j = 'a'; j <= 'z'; ++j)
			if (comp1.find(j) != string::npos && comp2.find(j) != string::npos)
				prio_sum += j - 'a' + 1;
		for (char j = 'A'; j <= 'Z'; ++j)
			if (comp1.find(j) != string::npos && comp2.find(j) != string::npos)
				prio_sum += j - 'A' + 27;
