...
def day_22_task_1():
	cards = []
	player2 = 0
	with open('day_22_crab_combat.in') as f:
		for line in f.readlines():
			if line.strip('\n') == 'Player 1:' or len(line.strip('\n')) < 1:
				continue
			elif line.strip('\n') == 'Player 2:':
				player2 = len(cards)
			else:
				cards += [int(line.strip('\n'))]

	while 0 < player2 < len(cards):
		if cards[0] > cards[player2]:
			cards = cards[1:player2] + [cards[0]] + cards[player2:]
			player2 += 1
		else:
			cards = cards[1:player2] + cards[player2 + 1:] + [cards[player2], cards[0]]
			player2 -= 1

	print(sum([(i + 1)*x for i, x in enumerate(cards[::-1])]))
...
player's score?
'''
def play_game(cards, player2):
	winner = 0
	game_history = []
	while 0 < player2 < len(cards):
		if (cards, player2) in game_history:
			return cards, len(cards)
		else:
			game_history += [(cards, player2)]

		if cards[0] < player2 and cards[player2] + player2 < len(cards):
			tmp_cards, tmp_player2 = play_game(cards[1:cards[0] + 1] + cards[player2 + 1:player2 + cards[player2] + 1], cards[0])
			if not tmp_player2:
				winner = 2
			else:
				winner = 1
		elif cards[0] > cards[player2]:
			winner = 1
		else:
			winner = 2
		if winner == 1:
...
def day_22_task_2():
	...
	cards, player2 = play_game(cards, player2)
