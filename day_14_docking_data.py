...
'''
def day_14_task_1():
	with open('day_14_docking_data.in') as f:
		mask = []
		mem = {}
		for line in f.readlines():
			...
			if data[0] == 'mask':
				mask = []
				for i, x in enumerate(data[2][::-1]):
					if x == '1':
						mask += [(int(2**int(i)), int(x))]
					elif x == '0':
						mask += [((2**36 - 1) ^ int(2**int(i)), int(x))]
			else:
				addr = int(data[0].split('[')[1][:-1])
				val = int(data[2])
				for x in mask:
					if x[1] == 1:
						val |= x[0]
	...
	with open('day_14_docking_data.in') as f:
		...
		for line in f.readlines():
			...
			if data[0] == 'mask':
				...
				for i, x in enumerate(data[2][::-1]):
					if x == '0':
						mask += [((2**36 - 1) ^ int(2**int(i)), x)]
					else:
						mask += [(int(2**int(i)), x)]
			else:
				orig_addr = int(data[0].split('[')[1][:-1])
				for x in mask:
					if x[1] == '1':
						orig_addr |= x[0]
					elif x[1] == 'X':
						orig_addr &= (2**36 - 1) ^ x[0]
				addrs = [orig_addr]
				for x in mask:
					if x[1] == 'X':
