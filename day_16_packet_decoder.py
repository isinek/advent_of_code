...
'''
def sum_versions(packet, n = 999):
	versions = 0
	while n and len(packet) and int(packet, 2):
		...
		if typeid == 4:
			...
			nn = int(packet[1:16], 2)
			v, p = sum_versions(packet[16:16 + nn])
			versions += v
			packet = packet[16 + nn:]
		elif packet[0] == '1':
			nn = int(packet[1:12], 2)
			v, p = sum_versions(packet[12:], nn)
			versions += v
			packet = p
...
def day_16_task_1():
	b = ''
	with open('day_16_packet_decoder.in') as f:
		line = f.readline()
	for x in line:
		b += '{0:04b}'.format(int(x, 16))

	print(sum_versions(b)[0])
...
'''
def execute_operation(typeid, data):
	val = 0
...
def process_packet(packet, n = 999):
	data = []
	while n and len(packet) and int(packet, 2):
		...
		if typeid == 4:
			...
			nn = int(packet[1:16], 2)
			d, p = process_packet(packet[16:16 + nn])
			packet = packet[16 + nn:]
			data += [execute_operation(typeid, d)]
		elif packet[0] == '1':
			nn = int(packet[1:12], 2)
			d, p = process_packet(packet[12:], nn)
			packet = p
			data += [execute_operation(typeid, d)]
