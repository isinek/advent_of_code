...
def day_4_task_1():
	...
	bingo_boards = []
	first_solved = (0, 0)
	with open('day_4_giant_squid.in') as f:
		nums = [int(n) for n in f.readline().split(',')]
		first_solved = (len(nums), 0)
		...
		for i, line in enumerate(f.readlines()):
			if not i%6:
				if len(bingo_boards):
					col_res, row_res = [], []
					for j in range(25):
						if not j%5 and len(col_res):
							if max(col_res) < first_solved[0]:
								first_solved = (max(col_res), sum([sum([nums[x] for x in r if x > max(col_res)]) for r in bingo_boards[-1]])*nums[max(col_res)])
							if max(row_res) < first_solved[0]:
								first_solved = (max(row_res), sum([sum([nums[x] for x in r if x > max(row_res)]) for r in bingo_boards[-1]])*nums[max(row_res)])
							col_res, row_res = [], []
						col_res += [bingo_boards[-1][j%5][j//5]]
						row_res += [bingo_boards[-1][j//5][j%5]]
				bingo_boards += [[]]
			else:
				bingo_boards[-1] += [[nums.index(int(n)) for n in line.split()]]
	print(first_solved[1])
			...
			if not i%6:
				if len(bingo_boards):
					...
					for j in range(25):
						...
						col_res += [bingo_boards[-1][j%5][j//5]]
						row_res += [bingo_boards[-1][j//5][j%5]]
					if first_solved[0] > last_solved[0]:
						last_solved = first_solved
				bingo_boards += [[]]
			else:
				bingo_boards[-1] += [[nums.index(int(n)) for n in line.split()]]
	print(last_solved[1])
